...
class ModelTrainer:
    ...
    def _forward_pass(self, model: torch.nn.Module,
        ...
        if training:
            ...
            outputs,  _, _, _, _, _,_, _,_,_,_, imputed_inputs = model(
    ...
    def train_epoch(self, model: torch.nn.Module,
        ...
        importances = model.gtacm.get_importance_manifolds(direction='all')
        print(f"[Training importances {importances}] Model weights reset to original\n")
        if self.is_classification:
    ...
    def validate_epoch(self, model: torch.nn.Module,
        ...
        with torch.no_grad():
            ...
            importances = model.gtacm.get_importance_manifolds(direction='all')
            print(f"[importances {importances}] Model weights reset to original\n")
            if self.is_classification:
    ...
    def evaluate_model(self, model_class: type, model_path: str,
        ...
        attended_cross_f_facit_w, attns_tsma_weights, interpolated_weights = [], [], []
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch_data in enumerate(progress_bar):
                ...
                (outputs,freq_w, attns_cross_f, attns_t_facit, attn_weights,
                 ...
                 interpolated_hidden, adaptive_factor, uncertainty,
        ...
        return (
            ...
            np.vstack(adaptive_factor_weights), np.vstack(uncertainty_weights),
    ...
    def inference(self, model_class: type, model_path: str,
        ...
        (real_inputs, imputed_inputs, freq_weights, freqs_curves_deca, freqs_curves_slope,
         ...
         adaptive_factor_weights, uncertainty_weights, y_true, y_pred) = evaluation_results
        ...
        return (
            ...
            [interpolated_weights],[adaptive_factor_weights], [uncertainty_weights]
        )
    ...
    def train_validate_evaluate(self, model_class: type, model: torch.nn.Module,
        ...
        """
        Execute complete training pipeline: train, validate, and evaluate.

        This is the main orchestration method that:
        1. Sets up logging and early stopping
        2. Runs training/validation loop
        3. Applies early stopping
        4. Evaluates best model on the test set
        5. Saves all results

        Args:
            model_class: Class constructor for model reinitialization
            model: Initialized model instance
            model_name: Identifier for this fold/run
            train_loader: Training data loader
            val_loader: Validation data loader
            test_loader: Test data loader
            rescale_params: Parameters for rescaling predictions
            save_path: Directory for saving results
            logger: Logger instance for tracking

        Returns:
            List containing all evaluation scores
        """
        # ================================================================
        # PHASE 1: Setup
        # ================================================================
        ...
        for epoch in range(self.num_epochs):
            ...
            if self.is_classification:
                ...
                extra_metrics = {"val_acc": val_acc,
                                 "val_loss_imp": val_mae}
                ...
                train_mae, train_loss, train_diff, train_mse = self.train_epoch(model, train_loader, epoch)
                val_mae, val_loss, val_diff, val_mse, val_mae_metric, val_targets, val_outputs = self.validate_epoch(
                    model, val_loader)
                ...
                log_message = (
                    f"Epoch {epoch + 1}/{self.num_epochs} - LR: {self.optimizer.param_groups[0]['lr']:.6f} | "
                    f"train - imp: {train_mae:.6f} , loss: {train_loss:.6f}, mse: {train_mse:.6f} , diff: {train_diff:.6f}| "
                    f"val - imp: {val_mae:.6f} , loss: {val_loss:.6f}, mse: {val_mse:.6f}, mae: {val_mae_metric:.6f}, diff: {val_mae_metric:.6f}"
                )
                logger.info(log_message)
    ...
    def _final_evaluation(self, model_class: type, model_path: str,
        ...
        (targets_list, outputs_list, real_inputs_list,
         ...
         attns_cross_f_facit_weights, attended_cross_f_facit_weights,
         interpolated_weights, adaptive_factor_weights, uncertainty) = self.inference(
            model_class, model_path, test_loader)

        # Unpack (these are already wrapped in single-item lists from inference)
        true_outputs = targets_list
        pred_outputs = outputs_list
        ...
        if self.is_classification:
            ...
            np.savez(
                ...
                adaptive_factor=adaptive_factor_weights
            )
            np.savez(
        ...
        del (true_outputs, pred_outputs, real_inputs, imputed_inputs,
             ...
             attns_cross_f_facit_weights, attended_cross_f_facit_weights,
             interpolated_weights,adaptive_factor_weights, uncertainty)
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
